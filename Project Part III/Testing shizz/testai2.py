...
def save_exhibition():
    ...
    desc_file_path = os.path.join("Ex_descriptions", f"{name}.txt")
    if not os.path.exists("Ex_descriptions"):
        os.makedirs("Ex_descriptions")
    ...
    try:
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (curator_id, researcher_id, desc_file_path, image_dest_path, 0, name)
        )
        conn.commit()
        conn.close()
...
Label(root, text="Curator ID:").grid(row=0, column=0, padx=10, pady=10)
curator_var = StringVar(root)
curator_var.set("1")  # Default value
OptionMenu(root, curator_var, "1", "2", "3").grid(row=0, column=1, padx=10, pady=10)

Label(root, text="Researcher ID:").grid(row=1, column=0, padx=10, pady=10)
researcher_var = StringVar(root)
researcher_var.set("1")  # Default value
OptionMenu(root, researcher_var, "1", "2", "3").grid(row=1, column=1, padx=10, pady=10)

Label(root, text="Name:").grid(row=2, column=0, padx=10, pady=10)
name_entry = Entry(root, width=30)
name_entry.grid(row=2, column=1, padx=10, pady=10)

Label(root, text="Likes:").grid(row=3, column=0, padx=10, pady=10)
likes_entry = Entry(root, width=30)
likes_entry.grid(row=3, column=1, padx=10, pady=10)

Label(root, text="Description:").grid(row=4, column=0, padx=10, pady=10)
desc_text = Text(root, width=30, height=5)
desc_text.grid(row=4, column=1, padx=10, pady=10)

Button(root, text="Import Image", command=import_image).grid(row=5, column=0, columnspan=2, pady=10)
Button(root, text="Save Exhibition", command=save_exhibition).grid(row=6, column=0, columnspan=2, pady=10)
